...
def passedTestsGX(testInfoGX):
    t1, t2 = getTestScores(testInfoGX)
    if t1 >= 0.8:
        if t2 >=0.9:
            return True
        else:
...
def passedTestsBX(testInfoBX):
    t1score, t2score = getTestScores(testInfoBX)
    t1month, t2month = getTestMonths(testInfoBX)
    if t1score >= 0.7:
        if t2month == 1 or t2month == 2:
            if t2score >= 0.98:
                return True
            else:
                return False
...
def determinePassedProducts(products):
    final = []
    for i in products:
        version = getProductVersion(i).upper()
        if version == "BX":
            if passedTestsBX(i):
...
# 1. it is necessary to use a loop while getting the test scores and test months becuase it is more efficient method for removing the serial#, and seperating the score/month from the rest of the values in the list. If the #f quality assurance tests in unknown for each product, a loop would come in handy, as we could simply find the length of the list and subtract 1, and then set our range from 1 to the list_length-1.
# 2. It is likely that i will use a for loop for the determinePassedProducts function rather than a while loop because in a while loop, there are more lines of code and the counter update has to have its own line of code. In the case where the counter is increasing/decreasing in increments more than 1 in a list, I might be more willing to use a while loop so that there is less confusion for the coder trying to interpret the loop.
# ```
