...
def getUserData(filename):
    #initialize username and password lists
    usernames = []
    passwords = []
    #put all data into list
    data = open(filename)
    data_list = data.readlines()
    #seperates usernames and password and adds to seperate lists
    for line in data_list:
        split_at_t = line.split('\t')
        usernames.append(split_at_t[0])
        passwords.append(split_at_t[1].strip('\n'))
    data.close()
    #returning usernames and passwords in a list
    return [usernames, passwords]


def exists(username, filename):
    #checks if the username exists in the file
    info = username in getUserData(filename) [0]
    return info
    
    
def createUser(username, password, filename):
    #add given username and password to file if they don't exist
...
def login(username, password, filename):
    #determine if correct password being used for username
    if exists(username, filename) == True:
        usernameIndex = getUserData(filename)[0].index(username)
        realPassword = getUserData(filename)[1][usernameIndex]
...
# 1. if the file was opened in write mode instead of append mode, it would have overwritten the data instead of adding new data, which means that some of the accounts would have gotten deleted. it may be possible to use the write function to do whataccomplish what appened does, but it would be much harded as you would need to know how many lines of data already exist so that you could add data after those lines.
# 2. To implement this dunction, we could simply create a new function that would take the entered username of password and check to see how long it is using the length feature and comapare it to 6. 
# 3. if we would want to encrypt the password, it would be benificial to encrypt it in the getUserData because this is where all the passwords and usernames are being sorted and all the other functions refer to this one.
# ```
